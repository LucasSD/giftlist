...
class GiftInstanceListViewTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        test_requester2 = User.objects.create(username="davidjones", password="password")
        ...
        # may be redundant
        test_giftinstance1 = GiftInstance.objects.create(gift=test_gift1, requester=test_requester1)
        test_giftinstance2 = GiftInstance.objects.create(gift=test_gift2, requester=test_requester2)
...
class GiftInstanceUpdateViewTest(TestCase):
    ...
    def test_redirect_if_not_logged_in(self):
        ...
        response = self.client.get(reverse("giftinstance-update", kwargs={"pk": test_giftinstance.id}))
        self.assertRedirects(response, f"/accounts/login/?next=/catalog/mygift/{test_giftinstance.id}")
